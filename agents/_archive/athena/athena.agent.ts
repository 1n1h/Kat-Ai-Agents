/**
 * Athena — The Actualizer
 * A standalone strategy & self-mastery coaching agent built on the Claude Agent SDK.
 *
 * Run standalone now; later register the same `athenaAgent` definition as a
...
const SYSTEM_PROMPT = readFileSync(
  new URL("./athena.system.md", import.meta.url),
  "utf8",
);

/**
 * Portable agent definition. This is the object you reuse when wiring Athena
 * into the orchestrator as a subagent (the orchestrator lists it under `agents`
 * and invokes it via the Task tool).
 */
export const athenaAgent = {
  description:
...
async function main() {
  ...
  console.log(
    "\nAthena — The Actualizer.  (type 'exit' to end)\n" +
